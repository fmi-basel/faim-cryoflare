...
######################## define output files ###################################
ctf_image=$rln_jobpath/${raw_movie/.mrcs/.ctf}
ctf_image_mrc=$rln_jobpath/${raw_movie/.mrcs/.ctf.mrc}
ctf_image_png=$rln_jobpath/${raw_movie/.mrcs/.ctf.png}
ctf_out=$rln_jobpath/${raw_movie/.mrcs/_gctf.out}
ctf_r1_log=$rln_jobpath/${raw_movie/.mrcs/_gctf_r1.log}
ctf_log=$rln_jobpath/${raw_movie/.mrcs/_gctf.log}
epa_log=$rln_jobpath/${raw_movie/.mrcs/_EPA.log}
aligned_micrograph_link=$rln_jobpath/${raw_movie/.mrcs/.mrc}
...
if FILES_MISSING; then
  ...
  gctf_params+=" --cs $ctf_input_c3"
  gctf_params+=" --ac 0.07"
  gctf_params+=" --dstep $dstep" 
  gctf_params+=" --astm 5000"
  ...
  defocus_params=" --defL `CALCULATE 0.9*$average_defocus`"
  defocus_params+=" --defH `CALCULATE 1.1*$average_defocus`"
...
astigmatism=`CALCULATE abs\($defocus_u-$defocus_v\)/2.0`
...
if [ -n "${CRYOFLARE_WRITE_RELION_31+x}" ]; then
  rln_header=(MicrographName CtfImage DefocusU DefocusV CtfAstigmatism DefocusAngle  CtfFigureOfMerit CtfMaxResolution rlnBeamTiltGroupName)
else
  rln_header=(MicrographName CtfImage DefocusU DefocusV DefocusAngle Voltage SphericalAberration AmplitudeContrast Magnification DetectorPixelSize CtfFigureOfMerit CtfMaxResolution rlnBeamTiltGroupName)
fi
if [ -n "$defocus_u" ] ; then
  if [ $phase_plate == "true" ] ; then
    ...
    if [ -n "${CRYOFLARE_WRITE_RELION_31+x}" ]; then
      RELION_WRITE_STAR $rln_alias $rln_starname "rln_header[@]" $aligned_micrograph ${ctf_image}:mrc $defocus_u $defocus_v $astigmatism $defocus_angle $ctf_epa_fom $ctf_max_resolution $template_id $phase_shift
    else
      RELION_WRITE_STAR $rln_alias $rln_starname "rln_header[@]" $aligned_micrograph ${ctf_image}:mrc \
                        $defocus_u $defocus_v $defocus_angle ${acceleration_voltage} $cs $amplitude_contrast $magnification $dstep $ctf_epa_fom $ctf_max_resolution $template_id $phase_shift
    fi
  else
    phase_shift=0
    if [ -n "${CRYOFLARE_WRITE_RELION_31+x}" ]; then
      RELION_WRITE_STAR $rln_alias $rln_starname "rln_header[@]" $aligned_micrograph ${ctf_image}:mrc $defocus_u $defocus_v $astigmatism $defocus_angle $ctf_epa_fom $ctf_max_resolution $template_id
    else
      RELION_WRITE_STAR $rln_alias $rln_starname "rln_header[@]" $aligned_micrograph ${ctf_image}:mrc \
                        $defocus_u $defocus_v $defocus_angle ${acceleration_voltage} $cs $amplitude_contrast $magnification $dstep $ctf_epa_fom $ctf_max_resolution $template_id
    fi
  fi
fi
